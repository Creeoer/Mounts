...
public class Commands implements CommandExecutor {
    ...
    private static final Set<String> VALID_COMMANDS = new HashSet<>(
	    Arrays.asList("rent", "give", "summon", "return", "reload", "remove"));
	...
	if ("mount".equalsIgnoreCase(cmd.getLabel())) {
	    if (args.length < 1) {
	    ...
	    if ("rent".equalsIgnoreCase(command)) {
		if (args.length == 2 && sender.hasPermission("mounts.admin")) {
		...
		if (!sender.hasPermission("mounts.admin")) {
		    sender.sendMessage(MOUNT_PREFIX + ChatColor.RED + "You have no permission to do this.");
		    return false;
		}
		main.reloadConfigAndMounts();
		sender.sendMessage(MOUNT_PREFIX + ChatColor.GREEN + "Config was successfully reloaded!");
	    }

	    if ("return".equalsIgnoreCase(command)) {
		if (!(sender instanceof Player)) {
		    sender.sendMessage(MOUNT_PREFIX + ChatColor.RED + "You must be a player to execute this command!");
		    return false;
		}
		...
		if (playerHorses.isEmpty()) {
		    sender.sendMessage(MOUNT_PREFIX + ChatColor.GRAY + "You don't have a horse to return");
		    return false;
		}
		...
		if (!mountType.isOwnable()) {
		    sender.sendMessage(MOUNT_PREFIX + ChatColor.GRAY
			    + "You can't return this horse as you are currently renting it");
		    return false;
		}
	    ...
	    if ("give".equalsIgnoreCase(command)) {
		if (!sender.hasPermission("mounts.give")) {
		    sender.sendMessage(
			    MOUNT_PREFIX + ChatColor.RED + "You have no permission to execute this command!");
		    return false;
		}
		...
		sender.sendMessage(
			MOUNT_PREFIX + ChatColor.GREEN + "You've given player " + player.getName() + " a horse!");
		player.sendMessage(MOUNT_PREFIX + ChatColor.AQUA + "You have been given a horse saddle");
	    }

	    if ("remove".equalsIgnoreCase(command) && sender.hasPermission("mounts.admin")) {
		if (args.length < 2) {
		...
		for (MountEntity mount : mounts) {
		    playerManager.removeHorseEntity(mount);
		    sender.sendMessage(MOUNT_PREFIX + ChatColor.GRAY + "Player horses removed");
		}
	    ...
	    if ("summon".equalsIgnoreCase(command) && sender instanceof Player) {
		Player player = (Player) sender;
